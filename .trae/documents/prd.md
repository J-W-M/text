...
| 角色 | 注册方式 | 核心权限 |
|------|----------|----------|
| 普通用户 | 手机号/邮箱注册 | 完整命理解读、个人档案存储、积分系统 |
| VIP用户 | 付费升级 | 深度详批、专属报告、优先AI响应 |
...
网站默认首页 = 登录/注册页面，除登录、注册页面外，所有业务页面强制校验登录态。
...
**安全机制**：
- 未登录访问任意业务页面 → 自动跳转登录页
- 后端所有业务接口统一鉴权，即便前端绕过限制，接口直接返回 401 拦截

### 3.2 用户测算流程

选择测算类型 → 录入生辰信息 → 系统校验排盘 → 选择服务等级 → 进入AI对话 → 获取命理解读 → 持续追问深化 → 生成完整报告 → 保存个人档案
